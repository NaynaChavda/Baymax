...
def predict():
    if request.method == 'POST':
        symptoms_input = request.form.get('symptoms')  # Comma-separated input
        symptoms_dropdown = request.form.getlist('symptomsDropdown')  # Selected dropdown options

        # Combine symptoms from both inputs
        symptoms = [s.strip() for s in symptoms_input.split(',')] if symptoms_input else []
        symptoms.extend(symptoms_dropdown)

        if not symptoms:
            message = "Please enter symptoms or select from the dropdown."
            return render_template('index.html', symptoms_dict=symptoms_dict, message=message)
        ...
        return render_template('index.html', symptoms_dict=symptoms_dict, predicted_disease=predicted_disease,
                               dis_des=dis_des, my_precautions=my_precautions, medications=medications,
                               my_diet=my_diet, workout=workout)
